...
import db.DBConnectionMgrVote;


public class VoteDao {
	private DBConnectionMgrVote pool;
	Connection con = null;
	...
	public VoteDao() {
		pool=DBConnectionMgrVote.getInstance();
	}
	...
	//설문 리스트에 투표목록 표시 (최근 입력된 투표가 먼저 보이게정렬)
	...
	public boolean updateCount(int num, String[] itemnum) {
		...
		try {
			...
			for(int i=0;i<itemnum.length;i++) {
				...
				int result = pstmt.executeUpdate(); //업데이트에 성공했으면 1반환
				if(result == 1)
					flag = true;
			}	
		} catch (Exception e) {
